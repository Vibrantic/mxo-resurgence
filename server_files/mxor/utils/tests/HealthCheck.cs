...
namespace mxor
{
	public class HealthCheck{
		...
		private int ConfigFileCheck(){
			
			try{
                config = new ServerConfig("Config.xml");
                
                config.LoadDbParams();
                config.LoadServerParams();
		...
		private int mysqlCheck(){
			try{
				...
				Output.Write("Failed. Debug error:"+e.Message+"\n");
				return 0;
			}
		}
		...
		public bool doTests(){
			...
			Output.Write("Md5 check ... ");
			totalPoints+= md5Check();
			
			Output.Write("CRC32 check ... ");
			totalPoints+= crcCheck();
			
			Output.Write("RSA (Decryption) check ... ");
			totalPoints+= rsaDecryptCheck();
			
			Output.Write("Twofish (Encryption) check ... ");
			totalPoints+= twofishEncrypt();
			
			Output.Write("Twofish (Decryption) check ... ");
			totalPoints+= twofishDecrypt();
			
			Output.Write("Config test check ... ");
			var res = ConfigFileCheck();
            if(res==0)
                return false;
            totalPoints += res;
            
			
			Output.Write("Mysql DB connection check ... ");
