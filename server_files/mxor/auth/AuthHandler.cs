...
namespace mxor.auth
{
    public class AuthHandler
    {
        ...
        {
            if (authClientSession?.IsConnected ?? false)
            {
                try
                {
                    authClientSession.SendAsync(data);
                }
                catch (ObjectDisposedException ex)
                {
                    Console.WriteLine($"[AuthHandler] SendAsync failed: {ex.Message}");
                }
            }
            else
            {
                Console.WriteLine("[AuthHandler] Attempted to send on disconnected session.");
            }
        }
        ...
        public void ProcessPacket(PacketReader reader, long receivedBytes)
        {
            ...
            if (!authClientSession.IsConnected)
            {
                Console.WriteLine("Connection lost before processing packet.");
                return;
            }
        ...
        public void ProcessGetPublicKey_Request(PacketReader reader)
        {
            ...
            SafeSend(responsePacket.ReturnFinalPacket());
        }
        ...
        public void ProcessHandleAuth_Request(PacketReader reader)
        {
            ...
            SafeSend(packetContent.ReturnFinalPacket());
        }
        ...
        public void ProcessHandleAuthChallenge_Response(PacketReader reader)
        {
            ...
            try
            {
                using var context = Store.matrixDbContext; // or however your DI/context works
                var user = context.Users
                    .Where(u => u.Username == wl.GetUsername())
                    .Single(u => u.Passwordmd5 == StringUtils.bytesToString_NS(md5.digest(passwordB)));

                // ...
            }
            catch (ObjectDisposedException ode)
            {
                Output.WriteLine("DB error: Connection disposed. " + ode.Message);
                SendAuthError((UInt32)AuthErrorCode.LTAS_ACCOUNTDOESNOTEXIST);
                return;
            }
            catch (Exception ex)
            {
                Output.WriteLine("Unhandled DB error: " + ex.Message);
                SendAuthError((UInt32)AuthErrorCode.LTAS_ACCOUNTDOESNOTEXIST);
                return;
            }
            ...
            if (Store.dbManager.AuthDbHandler.FetchWorldList(ref wl))
            {
                ...
                SafeSend(finalResponse.getBytes());
            }

            byte[] worldListPacket = {0x00, 0x00};

            status = -1; // trick
            SafeSend(worldListPacket);
        }
